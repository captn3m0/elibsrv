...
function getDefaultOutformat()
{
    ...
    // detect Firefox
    if (preg_match("/^.* Firefox\/.*$/", $ua) == 1) {
        return "html";
    }

    // detect IE
    if (preg_match("/^.* MSIE .*$/", $ua) == 1) {
        return "html";
    }
    ...
    // detect Chrome
    if (preg_match("/^.* Chrome\/.*$/", $ua) == 1) {
        return "html";
    }

    // detect Opera
    if (preg_match("/^.* Opera\/.*$/", $ua) == 1) {
        return "html";
    }

    // Detect Safari (as used by the kindle browser)
    if (preg_match("/^.* Safari\/.*$/", $ua) == 1) {
        return "html";
    }
...
function getEpubCoverFile($epubfile)
{
    ...
    for (;;) {
        ...
        if (substr($filename, -1) != '/') {
            ...
            if (
                preg_match("/^.*\/img1.jpg$/", $filename) == 1 &&
                $result == null
            ) {
                $result = $filename;
            }
...
function printnaventry($outformat, $title, $urlparams, $iconfile)
{
    ...
    $nav['title'] = $title;
    $nav['url'] =
        $_SERVER['PHP_SELF'] . '?f=' . $outformat . '&amp;' . $urlparams;
    $nav['icon'] = $iconfile;
    if ($outformat == "atom") {
...
function normalizefilename($f)
{
    ...
    for ($i = 0; $i < strlen($f); $i++) {
        if (ord($f[$i]) < 32 || in_array($f[$i], $weirdchars)) {
            $f[$i] = '_';
        }
    }
    return $f;
...
function printaqentry(
    ...
    $meta['format'] = $format;
    if ($prettyurls == 1) {
        // the epub link can have different forms, depending on the "pretty URLs" setting
        $meta['aqlink'] =
            "files/{$crc32}/" .
            rawurlencode(normalizefilename($author . " - " . $title)) .
            '.' .
            $format;
        $meta['aqlinkmobi'] =
            "files/{$crc32}/" .
        ...
        $meta['aqlink'] =
            $_SERVER['PHP_SELF'] . "?action=getfile&amp;query=" . $crc32;
        $meta['aqlinkmobi'] =
            $_SERVER['PHP_SELF'] . "?action=getmobi&amp;query=" . $crc32;
    }
    if (empty($kindlegenbin) || $format != 'epub') {
        $meta['aqlinkmobi'] = null;
    } // drop the mobi link if no kindlegenbin path is configured, or source file is not epub
    $meta['coverlink'] =
        $_SERVER['PHP_SELF'] . "?action=getcover&amp;query={$crc32}";
    $meta['thumblink'] =
        $_SERVER['PHP_SELF'] . "?action=getthumb&amp;query={$crc32}";
...
function titlesindex(
    ...
    $fieldslist =
        'crc32, title, author, description, language, publisher, pubdate, modtime, moddate, file, filesize, format';
    ...
    if (!empty($authorfilter)) {
        ...
        for ($i = 0; $i < $randcount; $i++) {
            $result = $db->query(
                'SELECT crc32 FROM books LIMIT 1 OFFSET ABS(random()) % (SELECT count(*) FROM books);'
            );
        ...
        $query =
            "SELECT {$fieldslist} FROM books WHERE crc32 IN (" .
            implode(',', $crclist) .
            ');';
    } elseif ($latest > 0) {
...
function authorsindex($outformat, $db)
{
    ...
    $query =
        "SELECT UPPER(SUBSTR(author,1,1)) AS fletter, COUNT(DISTINCT author) AS count FROM books GROUP BY fletter ORDER BY fletter;";
    $result = $db->query($query);
    ...
    while ($myrow = $result->fetchArray()) {
        ...
        printnaventry(
            $outformat,
            "{$fletter} ({$myrow['count']} entries)",
            "action=authorsl&amp;a=" . urlencode($fletter),
            null
        );
    }
    $result->finalize();
...
function authorsindex_letter($outformat, $db, $query)
{
    ...
    while ($myrow = $result->fetchArray()) {
        ...
        printnaventry(
            $outformat,
            $author,
            "action=titles&amp;a=" . urlencode($author),
            null
        );
    }
    $result->finalize();
...
function langsindex($outformat, $db)
{
    ...
    $query =
        "SELECT (CASE WHEN language = '' THEN 'unknown' ELSE language END) AS lang FROM books GROUP BY lang ORDER BY lang;";
    $result = $db->query($query);
    ...
    while ($myrow = $result->fetchArray()) {
        ...
        printnaventry(
            $outformat,
            $language,
            "action=titles&amp;l=" . urlencode($language),
...
function searchform($outformat)
{
    ...
    echo "  <Url type=\"application/atom+xml\" template=\"" .
        $_SERVER['PHP_SELF'] .
        "?action=titles&amp;f=" .
        $outformat .
        "&amp;query={searchTerms}\"/>\n";
...
{
    if (!empty($thumbdir)) {
        return $thumbdir . '/elibsrv-thumbcache-' . $crc32;
    }
    return '';
}
...
if (empty($latestdays) || $latestdays < 1) {
    $latestdays = 60;
}
$thumbheight = getconf($params, 'thumbheight');
if (empty($thumbheight) || $thumbheight < 1) {
    $thumbheight = 160;
}
...
if (empty($randcount) || $randcount < 1) {
    $randcount = 5;
}
...
if (
    $action == "getthumb" &&
    file_exists(computeThumbCacheFilename($thumbdir, $query))
) {
...
if ($action == "getfile") {
    ...
    if (empty($kindlegenbin)) {
        ...
        if ($localfile != null) {
            ...
            $localfilemobi =
                pathinfo($localfile['filename'], PATHINFO_DIRNAME) .
                '/' .
                pathinfo($localfile['filename'], PATHINFO_FILENAME) .
                '.mobi';
            // if doesn't exist yet, convert it using the kindlegen binary
            $kindleconvertcmd =
                $kindlegenbin . ' ' . escapeshellarg($localfile['filename']);
    ...
    if ($localfile != null) {
        if ($localfile['format'] == 'epub') {
            ...
            if ($coverfile != null) {
                $coverinzip =
                    "zip://" . $localfile['filename'] . "#" . $coverfile;
            }
        }
    }
    ...
    returnImageThumbnail(
        $coverinzip,
        $thumbheight,
        computeThumbCacheFilename($thumbdir, $query)
    );
} elseif ($action == "getcover") {
    $coverinzip = dirname(__FILE__) . "/images/nocover.png";
    $localfile = getLocalFilename($db, $query);
    if ($localfile != null) {
        if ($localfile['format'] == "epub") {
    ...
    /* return the cover file */
    header(
        "content-type: " . image_type_to_mime_type(exif_imagetype($coverinzip))
    );
    readfile($coverinzip);
